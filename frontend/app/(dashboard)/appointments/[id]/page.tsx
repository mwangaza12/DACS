...
// Re-export STATUS_TRANSITIONS from appointment-utils
...
export default function AppointmentDetailPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => updateAppointment(id, data),
    onSuccess: () => {
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl bg-card border border-border p-5">
            <div className="flex items-center gap-3 flex-wrap mb-4">
              ...
              <span className="text-xs text-text-muted font-body font-mono">{appt.appointmentId.slice(0, 16)}…</span>
            </div>

            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: <Calendar size={14} />, label: "Date", value: (() => { try { return format(parseISO(appt.appointmentDate), "EEEE, MMMM d yyyy"); } catch { return appt.appointmentDate; } })() },
                { icon: <Clock size={14} />,    label: "Time", value: `${appt.appointmentTime.slice(0,5)} – ${appt.endTime.slice(0,5)}` },
                { icon: <User size={14} />,       label: "Patient",  value: appt.patientId.slice(0, 16) + "…" },
                { icon: <Stethoscope size={14} />, label: "Doctor",   value: appt.doctorId.slice(0, 16) + "…" },
                ...
                <div key={label} className="p-3 rounded-xl bg-surface border border-border/60">
                  ...
                  <p className="text-sm text-text-primary font-body font-mono leading-tight">{value}</p>
                </div>
              ))}
            </div>
          </div>
